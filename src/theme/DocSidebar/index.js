...
function mutateSidebarCollapsingState(item, path) {
  ...
  switch (type) {
    ...
    //   item.collapsed = !anyChildItemsActive;
      ...
      const mapItems = items.map(childItem => mutateSidebarCollapsingState(childItem, path))
      console.log('mapItems =>', mapItems);

      const filterItems = mapItems.filter(val => val);
      console.log('filterItems =>', filterItems);
      
      const anyChildItemsActive = filterItems.length > 0;
